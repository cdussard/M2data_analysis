# -*- coding: utf-8 -*-
"""
Created on Fri Jul 28 15:27:22 2023
...
cond = "mainvib"
# Read the CSV files
df_global = pd.read_csv(path + "df_"+cond+"_estimate.csv", header=None, delimiter=",").iloc[1:, 1:].values
df_global_pval = pd.read_csv(path + "df_"+cond+"_pval.csv", header=None, delimiter=",").iloc[1:, 1:].values
...
axs.imshow(masked_global, extent=[0, 1, 0, 1],cmap="RdBu_r", aspect='auto',interpolation='none',vmin=vmin,vmax=vmax,label="agency") 
img = axs.imshow(masked_global, extent=[0, 1, 0, 1], cmap="RdBu_r", aspect='auto', interpolation='none', vmin=vmin, vmax=vmax, label="agency")
...
df_signif = df_global_pval[df_global_pval<0.05,]
df_signif.max()
...
beta_estimate_lim = df_global[24,75]#Wa voir si on fait le seuil 
