# -*- coding: utf-8 -*-
"""
Created on Wed Jan  3 16:31:04 2024
...
#=== QUAND ON FAIT LES DEUX MODELES SEPAREMENT =======================

path = "C:/Users/claire.dussard/OneDrive - ICM/Bureau/fig_brian/correlERD_agency_colorgrid/V3_2runs/"

# Read the CSV files
est_ag = pd.read_csv(path + "df_global_estimateFBseul_v3_2runs.csv", header=None, delimiter=",").iloc[1:, 1:].values
pval_ag = pd.read_csv(path + "df_global_pvalFBseul_v3_2runs.csv", header=None, delimiter=",").iloc[1:, 1:].values

path = "C:/Users/claire.dussard/OneDrive - ICM/Bureau/fig_brian/correlERD_NFperf_colorgrid/"

# Read the CSV files
est_perf = pd.read_csv(path + "df_global_estimateFBseul_v3_2runs_NFperf.csv", header=None, delimiter=",").iloc[1:, 1:].values
#pval_perf = pd.read_csv(path + "df_global_pvalFBseul_v3_2runs_NFperf.csv", header=None, delimiter=",").iloc[1:, 1:].values
pval_perf = pd.read_csv(path + "df_global_pvalFBseul_v3_2runs_NFperf_anova2.csv", header=None, delimiter=",").iloc[1:, 1:].values

pval_perf = pval_perf.astype(float)
pval_ag = pval_ag.astype(float)
est_perf = est_perf.astype(float)
est_ag = est_ag.astype(float)
...
masked_ag = np.ma.masked_where((p_ag_corr > pvalue) , df_subset_agency)
...
plt.yticks(np.linspace(1/(len(elecs)*2.5),1-1/(len(elecs)*2.5),len(elecs)),elecs.iloc[::-1])
img = axs[0].imshow(df_subset_agency, extent=extent,cmap=cmap, aspect='auto',interpolation='none',vmin=-0.02,vmax=0.02,label="agency") 
fig.colorbar(img, location = 'left')
img = axs[1].imshow(df_subset, extent=extent,cmap=cmap, aspect='auto',interpolation='none',vmin=-0.35,vmax=0.35,label="perf") 
fig.colorbar(img, location = 'right')
#contour = axs[0].contour(p_ag_corr,levels=[0.01,0.05], colors='black', linewidths=0.7, extent=extent, corner_mask='legacy', origin='upper',linestyles=["solid"],extend="min") 
#contour = axs[0].contour(p_perf_corr,levels=[0.01,0.05], colors='blue', linewidths=1, extent=extent, corner_mask='legacy', origin='upper',linestyles=["dashed"],extend="min") 
#contour = axs[1].contour(p_perf_corr,levels=[0.01,0.05], colors='black', linewidths=0.7, extent=extent, corner_mask='legacy', origin='upper',linestyles=["solid"],extend="min")
#contour = axs[1].contour(p_ag_corr,levels=[0.01,0.05], colors='blue', linewidths=1, extent=extent, corner_mask='legacy', origin='upper',linestyles=["dashed"],extend="min") 
plt.show()

#========= si on calcule l'interaction ds le modele


path = "C:/Users/claire.dussard/OneDrive - ICM/Bureau/rdom_scriptsData/allElecFreq_VSZero/versionJuin2023_elecFixed/"
elec_leg = pd.read_csv(path+"dcohen_mainIllusion.csv").iloc[:, 0]

path = "C:/Users/claire.dussard/OneDrive - ICM/Bureau/fig_brian/nouvelleFigureH4_Agency_ERD_perf/"
...
#img = axs.imshow(df_subset_perfag, extent=[0, 1, 0, 1],cmap=cmap, aspect='auto',interpolation='none',vmin=-0.14,vmax=0.14,label="agency") 
...
plot_topomapV2(8,30,df_subset_perfag,vmin,vmax,my_cmap)
...
#all the contours
fig, axs = plt.subplots(1,1, sharey=True,sharex=True, figsize=(14, 7),constrained_layout=True)
contour = axs.contour(df_pval_subset_perfag,levels=[0.01,0.05], colors='black', linewidths=1, extent=extent, corner_mask='legacy', origin='upper',linestyles=["solid","dashed"],extend="min") 
contour = axs.contour(df_pval_subset,levels=[0.01,0.05], colors='blue', linewidths=1, extent=extent, corner_mask='legacy', origin='upper',linestyles=["solid","dashed"],extend="min") 
contour = axs.contour(df_pval_subset_agency,levels=[0.01,0.05], colors='green', linewidths=1, extent=extent, corner_mask='legacy', origin='upper',linestyles=["solid","dashed"],extend="min") 
plt.savefig('test_3contours.png', transparent=True)

fig, axs = plt.subplots(1,1, sharey=True,sharex=True, figsize=(14, 7),constrained_layout=True)
contour = axs.contour(df_pval_subset_perfag,levels=[0.01], colors='black', linewidths=1, extent=extent, corner_mask='legacy', origin='upper',linestyles=["solid","dashed"],extend="min") 
contour = axs.contour(df_pval_subset,levels=[0.01], colors='blue', linewidths=1, extent=extent, corner_mask='legacy', origin='upper',linestyles=["solid","dashed"],extend="min") 
contour = axs.contour(df_pval_subset_agency,levels=[0.01], colors='green', linewidths=1, extent=extent, corner_mask='legacy', origin='upper',linestyles=["solid","dashed"],extend="min") 
plt.savefig('test_3contours_p0.01.png', transparent=True)
...
#contour = axs.contour(df_pval_subset_agency,levels=[0.01,0.05], colors='black', linewidths=1, extent=extent, corner_mask='legacy', origin='upper',linestyles=["solid","dashed"],extend="min") 
#fig.colorbar(img, location = 'right')
contour = axs.contour(df_pval_subset_perfag,levels=[0.05], colors='black', linewidths=1, extent=extent, corner_mask='legacy', origin='upper',linestyles=["solid","dashed"],extend="min") 
contour = axs.contour(df_pval_subset,levels=[0.05], colors='blue', linewidths=1, extent=extent, corner_mask='legacy', origin='upper',linestyles=["solid","dashed"],extend="min") 
contour = axs.contour(df_pval_subset_agency,levels=[0.05], colors='green', linewidths=1, extent=extent, corner_mask='legacy', origin='upper',linestyles=["solid","dashed"],extend="min") 
plt.savefig('test_3contours_p0.05.png', transparent=True)
# elecs = elec_leg 
# #fig, axs = plt.subplots(1,2, sharey=True,sharex=True, figsize=(14, 7),constrained_layout=True)
# freq_leg = np.arange(3,40,4)
# freq_leg_str =[str(f) for f in freq_leg]
...
#         ax.axhline(y=elecPos,color="dimgray",lw=0.25)
# plt.yticks(np.linspace(1/(len(elecs)*2.5),1-1/(len(elecs)*2.5),len(elecs)),elecs.iloc[::-1])
# img = axs[0].imshow(df_subset, extent=extent,cmap=cmap, aspect='auto',interpolation='none',vmin=-0.02,vmax=0.02,label="agency") 
# fig.colorbar(img, location = 'left')
# img = axs[1].imshow(df_subset_agency, extent=extent,cmap=cmap, aspect='auto',interpolation='none',vmin=-0.35,vmax=0.35,label="perf") 
# fig.colorbar(img, location = 'right')
# contour = axs[0].contour(df_pval_subset,levels=[0.01,0.05], colors='black', linewidths=0.7, extent=extent, corner_mask='legacy', origin='upper',linestyles=["solid","dashed"],extend="min") 
# contour = axs[1].contour(df_pval_subset_agency,levels=[0.01,0.05], colors='black', linewidths=0.7, extent=extent, corner_mask='legacy', origin='upper',linestyles=["solid","dashed"],extend="min")
